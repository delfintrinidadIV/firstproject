 ...
 * @since 1.0.0
 */
function silvermag_themepacific_optionsframework_admin_init() 
{
	...
	do_action('silvermag_themepacific_optionsframework_admin_init_before', array(
	...
	if (empty($smof_data['smof_init'])) { // Let's set the values if the theme's already been active
		...
		silvermag_themepacific_of_save_options(date('r'), 'smof_init');
		$smof_data = silvermag_themepacific_of_get_options();
		$options_machine = new silvermag_themepacific_Options_Machine($of_options);
	}

	do_action('silvermag_themepacific_optionsframework_admin_init_after', array(
			'of_options'		=> $of_options,
			'options_machine'	=> $options_machine,
			'smof_data'			=> $smof_data
		));
 ...
 * @since 1.0.0
 */
function silvermag_themepacific_optionsframework_add_admin() {
 
 
	   $of_page = add_theme_page( THEMENAME, 'Theme Options', 'edit_theme_options', 'tpcrn_framework', 'silvermag_themepacific_optionsframework_options_page');
	    $of_page2 = add_theme_page(  'Theme Docs', 'Theme Documentation', 'edit_theme_options', 'theme-documentaion', 'silvermag_themepacific_theme_doc');
    

	// Add framework functionaily to the head individually
	add_action("admin_print_scripts-$of_page", 'silvermag_themepacific_of_load_only');
	add_action("admin_print_styles-$of_page",'silvermag_themepacific_of_style_only');
...
function silvermag_themepacific_theme_doc(){
...
<div id="theme-doc" class="about-wrap">
	...
	<p> Silvermag is a Premium WordPress theme that has come with Magazine/blog style layout. It is an easy-to-customize and fully featured WordPress Theme. It has 3 columns, unlimited homepage Widgetized sections. Theme has come with WP Nav menus, Widgetized sidebars and Popular Widgets. Theme is built on advanced solid framework which includes lots of advanced features and functions. </p>
	
 <h3> Theme Documentaion</h3>
 <p>	The PHP code of the theme is licensed under the GPL license as is WordPress itself. You can read it here:  <a href="http://demo.themepacific.com/docs/silvermag/" target="_blank">Documentation</a> </p>
 ...
 * @since 1.0.0
 */
function silvermag_themepacific_optionsframework_options_page(){
 ...
 * @since 1.0.0
 */
function silvermag_themepacific_of_style_only(){
	...
	do_action('silvermag_themepacific_of_style_only_after');
}	
 ...
 * @since 1.0.0
 */
function silvermag_themepacific_of_load_only() 
{
	...
	do_action('silvermag_themepacific_of_load_only_after');
 ...
 * @since 1.0.0
 */
function silvermag_themepacific_of_ajax_callback() 
{
	...
	$all = silvermag_themepacific_of_get_options();
	...
	if($save_type == 'upload')
	{
		...
		$uploaded_file = wp_handle_upload($filename,$override);
			...
			silvermag_themepacific_of_save_options($upload_image);
	...
	elseif($save_type == 'image_reset')
	{
			...
			silvermag_themepacific_of_save_options($delete_image ) ;
	...
	elseif($save_type == 'backup_options')
	{
		...
		silvermag_themepacific_of_save_options($backup, BACKUPS) ;
	...
	elseif($save_type == 'restore_options')
	{
		...
		$smof_data = silvermag_themepacific_of_get_options(BACKUPS);
