 ...
 * @author      Syamil MJ
 */
class silvermag_themepacific_Options_Machine {
	...
	function __construct($options) {
		...
		$return = $this->silvermag_themepacific_optionsframework_machine($options);
	 ...
	 */
	static function silvermag_themepacific_sanitize_option( $value ) {
		$defaults = array(
			"name" 		=> "",
			"desc" 		=> "",
			"id" 		=> "",
	 ...
	 * @return array
	 */
	public static function silvermag_themepacific_optionsframework_machine($options) {
		global $smof_output, $smof_details, $smof_data;
		if (empty($options))
			return;
		if (empty($smof_data))
			$smof_data = silvermag_themepacific_of_get_options();
		$themepacific_data = $smof_data;
		...
		do_action('silvermag_themepacific_optionsframework_machine_before', array(
				'options'	=> $options,
				'smof_data'	=> $smof_data,
			));
		if ($smof_output != "") {
		...
		foreach ($options as $value) {
			...
			if ($value['type'] != "heading")
				$value = self::silvermag_themepacific_sanitize_option($value);
			...
			/* condition start */
			if(!empty($smof_data) || !empty($themepacific_data)){
			...
			switch ( $value['type'] ) {
				...
				case 'select':
					...
					if(!isset($value['mod'])) $value['mod'] = '';
					if($value['mod'] == 'mini') { $mini = 'mini';}
					$output .= '<div class="select_tp-site-wrap ' . $mini . '">';
				...
				case 'typography':
					...
					if(isset($typography_stored['size'])) {
						$output .= '<div class="select_tp-site-wrap typography-size" original-title="Font size">';
					...
					if(isset($typography_stored['height'])) {
						...
						$output .= '<div class="select_tp-site-wrap typography-height" original-title="Line height">';
					...
					if(isset($typography_stored['face'])) {
						...
						$output .= '<div class="select_tp-site-wrap typography-face" original-title="Font family">';
		...
		'Zeyada' => 'Zeyada',
					...
					if(isset($typography_stored['style'])) {
						...
						$output .= '<div class="select_tp-site-wrap typography-style" original-title="Font style">';
						$output .= '<select class="of-typography of-typography-style select" name="'.$value['id'].'[style]" id="'. $value['id'].'_style">';
						$styles = array('400'=>'400',
										'300'=>'300',
										'500'=>'500',
										'700'=>'700');
				...
				case 'border':
					...
					$output .= '<div class="select_tp-site-wrap border-width">';
					...
					/* Border Style */
					$output .= '<div class="select_tp-site-wrap border-style">';
				...
				case 'images':
					...
					$i++;
						...
						$output .= '<img src="'.$option.'" alt="" class=" of-radio-img-img '. $key .' '. $selected .'" onClick="document.getElementById(\'of-radio-img-'. $value['id'] . $i.'\').checked = true;" />';
						$output .= '</span>';				
					}
				...
				case 'slider':
					...
					if ($count < 2) {
						$oldorder = 1;
						$order = 1;
						$output .= silvermag_themepacific_Options_Machine::silvermag_themepacific_optionsframework_slider_function($value['id'],$value['std'],$oldorder,$order);
					} else {
						$i = 0;
				...
				case 'sorter':
				    ...
				    $sortlists = isset($themepacific_data[$value['id']]) && !empty($themepacific_data[$value['id']]) ? $themepacific_data[$value['id']] : $value['std'];
				...
				case 'backup':
					...
					$backup = silvermag_themepacific_of_get_options(BACKUPS);
					$init = silvermag_themepacific_of_get_options('smof_init');
					...
					$output .= '<p><strong>'. __('Last Backup:','silvermag').'<span class="backup-log">'.$log.'</span></strong></p></div>'."\n";
				...
				case 'select_google_font':
					$output .= '<div class="select_tp-site-wrap">';
				...
				case "media":
					...
					$output .= silvermag_themepacific_Options_Machine::silvermag_themepacific_optionsframework_media_uploader_function($value['id'],$u_val, $value['mod']);
					
				break;
				
			}

			do_action('silvermag_themepacific_optionsframework_machine_loop', array(
					'options'	=> $options,
					'smof_data'	=> $smof_data,
					'defaults'	=> $defaults,
					'counter'	=> $counter,
			...
			if ( $value['type'] != 'heading') { 
				...
				if(!empty($value['desc']))
					$explain_value = '<span class="explain"  original-title="'. $value['desc'] .'"></span>'."\n";
					else
					$explain_value = '';					
				} 
		...
		if ($update_data == true) {
			silvermag_themepacific_of_save_options($smof_data);
		}
		
	    $output .= '</div>';

	    do_action('silvermag_themepacific_optionsframework_machine_after', array(
					'options'		=> $options,
					'smof_data'		=> $smof_data,
					'defaults'		=> $defaults,
					'counter'		=> $counter,
	 ...
	 * @return string
	 */
	public static function silvermag_themepacific_optionsframework_media_uploader_function($id,$std,$mod){
	 ...
	 * @return string
	 */
	public static function silvermag_themepacific_optionsframework_slider_function($id,$std,$oldorder,$order){
